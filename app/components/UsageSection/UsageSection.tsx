...
function UsageSectionContent({testimonials} : Props) {
    const textCn = useTextStyles();

    const [sortByName, setSortByName] = useState(false);

    useLayoutEffect(() => {
        const sortByName = localStorage.getItem('kotlin-testimonials-order');;
        setSortByName(sortByName === "name");
    }, []);

    useEffect(() => {
        const sortOrder = sortByName ? "name" : "default";
        localStorage.setItem('kotlin-testimonials-order', sortOrder);
    }, [sortByName]);
...
export function UsageSection({testimonials} : Props) {
    return (
